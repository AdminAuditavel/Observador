...
const AirportHome: React.FC<AirportHomeProps> = ({ onOpenWeather }) => {
  ...
  const openAuthorPhoto = (e: React.MouseEvent | React.KeyboardEvent, url: string) => {
    // garante que o clique/tecla não propague
      ...
      <main className="flex flex-col gap-6 -mt-4 relative z-10 px-4">
        ...
        <section className="flex flex-col gap-4">
          ...
          {posts.map(post => (
            <article
              key={post.id}
              className="bg-surface-dark rounded-xl overflow-hidden border border-white/5 shadow-lg transition-all cursor-default group"
            >
              <div className="relative h-48 w-full bg-gray-800 overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 w-full p-4 z-10 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 z-20 no-nav">
                    ...
                    <button
                      className="text-gray-400 no-nav"
              ...
              {/* Título / conteúdo do post (fora da área clicável da imagem) */}
              <div className="p-3">
